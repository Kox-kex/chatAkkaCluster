import Chat.{BroadcastMessage, Command, Message, Subscribe}
...
object Chat {
  ...
  case class Subscribe() extends Command with CborSerializable
  ...
  override def onMessage(msg: Command): Behavior[Command] = {
    msg match {
      ...
      Platform.runLater(new Runnable {
          override def run(): Unit = ChatPane.txtAreaDisplay.appendText(s"[$name]: $msg\n")
        })
        Behaviors.same

        //подписка на рассылку сообщений общего чата
      case Subscribe() =>
